<div class="bg-white shadow rounded-lg overflow-hidden border border-gray-200 mb-4"
    ...
    <div class="px-4 py-3 bg-slate-100 flex flex-col justify-between items-start">
        ...
        <span class="text-xs text-gray-500">{{ $reporte->tiempo_transcurrido }} - {{ $reporte->tecnico->name }}</span>
    </div>
        ...
        <ul class="space-y-3">
            @foreach ($reporte->comentarios as $c)
                <li class="flex items-start gap-3">
                    <div class="h-8 w-8 rounded-full bg-slate-200 flex items-center justify-center text-slate-600 text-sm font-semibold">
                        {{ mb_substr($c->user->name ?? 'U', 0, 1) }}
                    </div>
                    <div class="flex-1 bg-gray-100 p-2 rounded-2xl">
                        <div class="flex items-center gap-2">
                            <span class="font-semibold text-sm text-gray-800">{{ $c->user->name ?? 'Usuario' }}</span>
